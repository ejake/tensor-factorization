...
//////////////////////
    ...
    namespace {
    struct __struct_compiled_op_5d348737efd2891af39315069a44b3e1 {
        PyObject* __ERROR;
...
PyObject* storage_V1;
        ...
        __struct_compiled_op_5d348737efd2891af39315069a44b3e1() {
        ...
        ~__struct_compiled_op_5d348737efd2891af39315069a44b3e1(void) {
            cleanup();
        }
...
// Op class CGer
    ...
    if (PyArray_DIMS(V3)[1] != PyArray_DIMS(V9)[0])
    {
        PyErr_SetString(PyExc_ValueError,
    ...
    if (!1
        || (PyArray_STRIDES(V3)[0] < 0)
        || (PyArray_STRIDES(V3)[1] < 0)
        || ((PyArray_STRIDES(V3)[0] != elemsize)
            && (PyArray_STRIDES(V3)[1] != elemsize)))
    {
        npy_intp dims[2];
        ...
        if ((NULL == V1)
            ...
            || ((PyArray_STRIDES(V1)[0] != elemsize)
                && (PyArray_STRIDES(V1)[1] != elemsize)))
        {
            Py_XDECREF(V1);
        ...
        if (PyArray_DESCR(V1)->type_num == NPY_FLOAT)
        {
            ...
            const float * xdata = (float*)PyArray_DATA(V7);
            const float * ydata = (float*)PyArray_DATA(V9);
            const float * adata = (float*)PyArray_DATA(V5);
            const float alpha = adata[0];
            float tmp, xx;
            int Ai = PyArray_STRIDES(V3)[0]/sizeof(float);
            int Aj = PyArray_STRIDES(V3)[1]/sizeof(float);
            int Zi = PyArray_STRIDES(V1)[0]/sizeof(float);
            int Zj = PyArray_STRIDES(V1)[1]/sizeof(float);
            int xi = PyArray_STRIDES(V7)[0]/sizeof(float);
            int yj = PyArray_STRIDES(V9)[0]/sizeof(float);
        ...
        else if (PyArray_DESCR(V1)->type_num == NPY_DOUBLE)
        {
            ...
            const double * xdata = (double*)PyArray_DATA(V7);
            const double * ydata = (double*)PyArray_DATA(V9);
            const double * adata = (double*)PyArray_DATA(V5);
            const double alpha = adata[0];
            double tmp, xx;

            int Ai = PyArray_STRIDES(V3)[0]/sizeof(double);
            int Aj = PyArray_STRIDES(V3)[1]/sizeof(double);
            int Zi = PyArray_STRIDES(V1)[0]/sizeof(double);
            int Zj = PyArray_STRIDES(V1)[1]/sizeof(double);
            int xi = PyArray_STRIDES(V7)[0]/sizeof(double);
            int yj = PyArray_STRIDES(V9)[0]/sizeof(double);
            for (int i = 0; i < dims[0]; ++i)
            {
                xx = alpha * xdata[xi * i];
                for (int j = 0; j < dims[1]; ++j)
    ...
    else
    {
        ...
        {
            int Nz0 = PyArray_DIMS(V1)[0];
            int Nz1 = PyArray_DIMS(V1)[1];
            int Sx = PyArray_STRIDES(V7)[0] / elemsize;
             ...
             *  - they are not 0.
             */
            int Sz0 = (Nz0 > 1) ? (PyArray_STRIDES(V1)[0] / elemsize) : (Nz1 + 1);
            int Sz1 = (Nz1 > 1) ? (PyArray_STRIDES(V1)[1] / elemsize) : (Nz0 + 1);

            dtype_V7* x_data = (dtype_V7*) PyArray_DATA(V7);
            dtype_V9* y_data = (dtype_V9*) PyArray_DATA(V9);
            ...
            if (PyArray_STRIDES(V1)[0] == elemsize)
            {
                if (PyArray_DESCR(V1)->type_num == NPY_FLOAT)
                {
                    //fprintf(stderr, "A\n");
        ...
        goto __label_11;}
            ...
            else if (PyArray_STRIDES(V1)[1] == elemsize)
            {
                if (PyArray_DESCR(V1)->type_num == NPY_FLOAT)
                {
                    float alpha = ((dtype_V5*)(PyArray_DATA(V5)))[0];
...
double __DUMMY_2;
        ...
        static int __struct_compiled_op_5d348737efd2891af39315069a44b3e1_executor(__struct_compiled_op_5d348737efd2891af39315069a44b3e1* self) {
            return self->run();
        }

        static void __struct_compiled_op_5d348737efd2891af39315069a44b3e1_destructor(void* executor, void* self) {
            delete ((__struct_compiled_op_5d348737efd2891af39315069a44b3e1*)self);
        }
...
static PyObject * instantiate(PyObject * self, PyObject *argtuple) {
  ...
  __struct_compiled_op_5d348737efd2891af39315069a44b3e1* struct_ptr = new __struct_compiled_op_5d348737efd2891af39315069a44b3e1();
  ...
  PyObject* thunk = PyCObject_FromVoidPtrAndDesc((void*)(&__struct_compiled_op_5d348737efd2891af39315069a44b3e1_executor), struct_ptr, __struct_compiled_op_5d348737efd2891af39315069a44b3e1_destructor);
  return thunk; }
...
PyMODINIT_FUNC init5d348737efd2891af39315069a44b3e1(void){
   import_array();
   (void) Py_InitModule("5d348737efd2891af39315069a44b3e1", MyMethods);
}
