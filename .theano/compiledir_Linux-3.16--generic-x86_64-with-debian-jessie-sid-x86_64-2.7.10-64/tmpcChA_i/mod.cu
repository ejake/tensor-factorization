...
//////////////////////
            ...
            static __global__ void kernel_reduce_ccontig_node_fb5bc112d1778d5cf2ea21dbb43e7346_0(
        ...
        TODO: find why it don't work or put the GPU compute capability into the version
            ...
            static __global__ void kernel_reduce_11_node_fb5bc112d1778d5cf2ea21dbb43e7346_0(
                    const int d0,
                    const int d1,
                    ...
                    float * Z)
            {
                ...
                for (int i0 = threadIdx.y; i0 < d0; i0 += blockDim.y)
                {
                    ...
                    {
                        myresult = myresult + A[i0 * sA0 + i1 * sA1] * A[i0 * sA0 + i1 * sA1];;
                    }
                }
    ...
    namespace {
    struct __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346 {
        PyObject* __ERROR;
...
PyObject* storage_V1;
        ...
        __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346() {
        ...
        ~__struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346(void) {
            cleanup();
        }
...
if(CudaNdarray_is_c_contiguous(V3)){

        {
          if(CudaNdarray_SIZE(V3)==0){
            ...
            dim3 n_blocks(1);
            if (verbose) printf("running kernel_reduce_ccontig_node_fb5bc112d1778d5cf2ea21dbb43e7346_0"
                                " n_threads.x=%d, size=%d, ndim=%d\n",
                                n_threads.x,CudaNdarray_SIZE(V3),V3->nd);
            int n_shared = sizeof(float) * n_threads.x;
            kernel_reduce_ccontig_node_fb5bc112d1778d5cf2ea21dbb43e7346_0<<<n_blocks, n_threads, n_shared>>>(
                    CudaNdarray_SIZE(V3),
            ...
            if (cudaSuccess != sts)
            {
                PyErr_Format(PyExc_RuntimeError,
                    ...
                    "kernel_reduce_ccontig_node_fb5bc112d1778d5cf2ea21dbb43e7346_0",
...

        {
            ...
            n_threads.y -= 1;
            if (n_threads.y > CudaNdarray_HOST_DIMS(V3)[0])
                n_threads.y = CudaNdarray_HOST_DIMS(V3)[0];

            dim3 n_blocks(1);
            
            if (verbose)
                printf("running kernel_reduce_11_node_fb5bc112d1778d5cf2ea21dbb43e7346_0\n");
            ...
            if (verbose>1)
                printf("n_threads.x=%d, n_threads.y=%d, n_threads.z=%d,"
                       ...
                       " nb_block=%d, n_shared=%d, shape=(%d,%d)\n",
                                  n_threads.x,n_threads.y,n_threads.z,
                                  n_threads.x*n_threads.y*n_threads.z,
                                  n_blocks.x,n_blocks.y,
                                  n_blocks.x*n_blocks.y, n_shared, CudaNdarray_HOST_DIMS(V3)[0],CudaNdarray_HOST_DIMS(V3)[1]);
            kernel_reduce_11_node_fb5bc112d1778d5cf2ea21dbb43e7346_0<<<n_blocks, n_threads, n_shared>>>(
            ...
            if (cudaSuccess != sts)
            {
                PyErr_Format(PyExc_RuntimeError,
                    ...
                    " (grid: %i x %i; block: %i x %i x %i)"
                    " shape=(%d,%d) \n",
                    "kernel_reduce_11_node_fb5bc112d1778d5cf2ea21dbb43e7346_0",
                    cudaGetErrorString(sts),
                    n_blocks.x,
                    n_blocks.y,
                    n_threads.x,
                    n_threads.y,
                    n_threads.z,
                    CudaNdarray_HOST_DIMS(V3)[0],CudaNdarray_HOST_DIMS(V3)[1]);
                {
        __failure = 5;
        if (!PyErr_Occurred()) {
            PyErr_SetString(PyExc_RuntimeError,
...
double __DUMMY_2;
        ...
        static int __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346_executor(__struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346* self) {
            return self->run();
        }

        static void __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346_destructor(void* executor, void* self) {
            delete ((__struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346*)self);
        }
...
static PyObject * instantiate(PyObject * self, PyObject *argtuple) {
  ...
  __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346* struct_ptr = new __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346();
  if (struct_ptr->init( PyTuple_GET_ITEM(argtuple, 0),PyTuple_GET_ITEM(argtuple, 1),PyTuple_GET_ITEM(argtuple, 2) ) != 0) {
    delete struct_ptr;
    return NULL;
  }
  PyObject* thunk = PyCObject_FromVoidPtrAndDesc((void*)(&__struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346_executor), struct_ptr, __struct_compiled_op_fb5bc112d1778d5cf2ea21dbb43e7346_destructor);
  return thunk; }
...
PyMODINIT_FUNC initfb5bc112d1778d5cf2ea21dbb43e7346(void){
   (void) Py_InitModule("fb5bc112d1778d5cf2ea21dbb43e7346", MyMethods);
}
