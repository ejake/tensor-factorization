...
def CNMF(KX,epochs,W,G,calculateErrors = False):
    ...
    for i in xrange(epochs):
        print i
        n = dot(KXP,W) + dot(G.T,dot(W.T,dot(KXN,W)))
        d = dot(KXN,W) + dot(G.T,dot(W.T,dot(KXP,W)))
        f = np.sqrt(n/d)
        G = G * f
...
def Error(KX,W,G):
    E1 = np.trace(KX)
    E2 = np.trace(np.dot(np.dot(KX,W),G))
    E3 = np.trace(np.dot(G.T,np.dot(W.T,np.dot(np.dot(KX,W),G))))
    return E1 + E3 - (2.0 * E2)