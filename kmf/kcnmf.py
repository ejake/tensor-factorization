...
class KCNMF(NMF):
    ...
    def __init__(self, data, num_bases=4):
        ...
        (self._data_dimension, self._num_samples) = data.shape
        self._EPS = 10**-8# some small value
    ...
    def init_h(self):
        if not hasattr(self, 'H'):
            ...
            km = Kmeans(self.X[:,:], num_bases=self._num_bases)
    ...
    def frobenius_norm(self):
        ...
        if hasattr(self,'H') and hasattr(self,'W') and not scipy.sparse.issparse(self.X):
                err = np.linalg.norm(self.X[:,:] - np.dot(self.W, self.H))/np.linalg.norm(self.X[:,:])
        else:
            err = -123456
    ...
    def factorize(self, niter=10, compute_w=True, compute_h=True,
        ...
        XtX = self.X[:,:]#edited from original
