...
class NMF():
    ...
    data : array_like, shape (_data_dimension, _num_samples)
        the input data
    cdata : array_like, shape (_data_dimension, _num_samples)
        the input data (data split with only complete data)
    idata : array_like, shape (_data_dimension, _num_samples)
        the input data (data split with only incomplete data)
    num_bases: int, optional
        Number of bases to compute (column rank of W and row rank of H).
        4 (default)
    t_valid : Type of validation 1 (default)
        1 regular validation data is used to compute error and measure convergence
        2 cdata is used to compute error and measure convergence
    ...
    def __init__(self, data, num_bases=4, t_valid = 1, cdata, idata): #Modified by me
        ...
        # set variables
        self.data = data
        self.cdata = cdata #Complete data (Add by me)
        self.udata = idata #Incomplete data (Add by me)
        self._num_bases = num_bases
        self._t_valid = t_valid #Type of validation (Add by me)
        if self._t_valid == 1:
            self.X = self.data
        else if self._t_valid == 2:
            self.X = self.cdata
        else:
            self.X = self.data
